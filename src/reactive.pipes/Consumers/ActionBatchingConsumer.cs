...
namespace reactive.pipes.Consumers
{
	...
	public class ActionBatchingConsumer<T> : BatchingConsumer<T>
	{
		...
		public ActionBatchingConsumer(Action<IList<T>> @delegate) => DelegateForAction(@delegate);
		...
		public ActionBatchingConsumer(Func<IList<T>, bool> @delegate) => _delegate = @delegate;
		...
		public ActionBatchingConsumer(Func<IList<T>, bool> @delegate, int itemsPerBatch, TimeSpan orInterval) :
			base(itemsPerBatch, orInterval) => _delegate = @delegate;
