...
namespace System.Threading.Tasks.Schedulers
{
	...
	public sealed class QueuedTaskScheduler : TaskScheduler, IDisposable
	{
		...
		public QueuedTaskScheduler(int threadCount) : this(threadCount, string.Empty, false, ThreadPriority.Normal,
			ApartmentState.MTA, 0, null, null)
		{
		}
			...
			Action threadFinally = null)
		{
			...
			if (threadCount == 0) _concurrencyLevel = Environment.ProcessorCount;
			...
			foreach (var queueGroup in _queueGroups)
			{
				...
				foreach (var i in queues.CreateSearchOrder())
				{
					...
					if (items.Count > 0)
					{
						...
						if (queueForTargetTask._disposed && items.Count == 0) RemoveQueue_NeedsLock(queueForTargetTask);
