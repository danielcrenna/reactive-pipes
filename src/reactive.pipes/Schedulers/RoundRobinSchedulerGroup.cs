...
namespace System.Threading.Tasks.Schedulers
{
	...
	public sealed class RoundRobinSchedulerGroup
	{
		...
		public ReadOnlyCollection<TaskScheduler> Schedulers
		{
			get
			{
				lock (_queues) return new ReadOnlyCollection<TaskScheduler>(_queues.Cast<TaskScheduler>().ToArray());
			}
		}
		...
		private void NotifyNewWorkItem()
		{
			...
			ThreadPool.QueueUserWorkItem(_ =>
			{
				...
				lock (_queues)
				{
					...
					var searchOrder = Enumerable.Range(_nextQueue, _queues.Count - _nextQueue)
