...
namespace reactive.pipes.benchmarks
{
	...
	public class ConsumerBenchmarks
	{
		...
		{
			for (var i = 0; i < Trials; i++) _default.Publish(new BaseMessage());
		}

		[Benchmark]
		public void ReactivePipes_single_subscription_async()
		{
			for (var i = 0; i < Trials; i++) _default.PublishAsync(new BaseMessage());
		}

		[Benchmark]
		public void ReactivePipes_single_subscription_unsafe_wait_async()
		{
			for (var i = 0; i < Trials; i++) _unsafe.PublishAsync(new BaseMessage());
		}

		[Benchmark]
		public void ReactivePipes_manifold_subscription_sequential()
		{
			for (var i = 0; i < Trials; i++) _manifold.Publish(new SubMessage());
		}

		[Benchmark]
		public void ReactivePipes_manifold_subscription_async()
		{
			for (var i = 0; i < Trials; i++) _manifold.PublishAsync(new SubMessage());
		}
