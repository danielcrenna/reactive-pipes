...
namespace reactive.pipes.scheduled
{
	public class ScheduledProducer : IProduce<IEnumerable<ScheduledTask>>, IDisposable
	{
		...
		private static readonly IDictionary<Type, HandlerMethods> MethodCache =
			new ConcurrentDictionary<Type, HandlerMethods>();
		...
		{
			Parallel.ForEach(_pending.Where(entry => entry.Value.OnHalt != null),
		...
		private void WithPendingTasks(IEnumerable<ScheduledTask> scheduledTasks)
		{
			...
			{
				var scheduler = AcquireScheduler(scheduledTask);
				var cancel = new CancellationTokenSource();
				var taskFactory = _factories[scheduler];
				var task = taskFactory.StartNew(() => { AttemptTask(scheduledTask); }, cancel.Token);
			...
			{
				var scheduledTask = subjects[performer.Key];
		...
		internal bool AttemptTask(ScheduledTask task, bool persist = true)
		{
			...
			if (persist) UpdateTask(task, success, exception);
		...
		private bool AttemptCycle(ScheduledTask task, out Exception exception)
		{
			...
			if (!success) task.RunAt = DateTimeOffset.UtcNow + _settings.IntervalFunction(task.Attempts);
			return success;
		}
		...
		private Handler CreateOrGetHandler(ScheduledTask task)
		{
			...
			if (!HandlerCache.TryGetValue(handlerInfo, out handler))
			{
				...
				var type = _settings.TypeResolver.FindTypeByName(typeName);
				if (type != null)
				{
					object instance;
